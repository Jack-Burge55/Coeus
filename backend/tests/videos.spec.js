...
describe("/api/v1/videos", () => {
  ...
  it("Should delete a valid video", async () => {
    const res = await request(app).delete("/api/v1/videos").send({"videoId": videoId}).set({ authorisation: `Bearer ${token}` });
    expect(res.status).toBe(200);
  ...
  it("Should return an error if an invalid video is tried to be deleted", async () => {
    const res = await request(app).delete("/api/v1/videos").send({"videoId": `2${videoId.slice(1)}`}).set({ authorisation: `Bearer ${token}` });
    expect(res.status).toBe(400);
